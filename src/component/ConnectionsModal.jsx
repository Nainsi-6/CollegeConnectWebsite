...
const ConnectionsModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] lg:max-h-[80vh] overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg lg:text-xl font-semibold text-indigo-700">Your Connections</h2>
        ...
        <div className="px-2 lg:px-4 py-2">
          <Tab.Group>
            <Tab.List className="flex space-x-1 rounded-xl bg-indigo-100 p-1">
              <Tab
                className={({ selected }) =>
                  `w-full rounded-lg py-2 lg:py-2.5 text-xs lg:text-sm font-medium leading-5 ${
                    selected ? "bg-indigo-600 text-white shadow" : "text-indigo-700 hover:bg-indigo-200"
                  }`
                }
              >
              ...
              >
                Requests ({connections.pending.length})
              </Tab>
              <Tab
                className={({ selected }) =>
                  `w-full rounded-lg py-2 lg:py-2.5 text-xs lg:text-sm font-medium leading-5 ${
            ...
            <Tab.Panels className="mt-2">
              {/* Followers Panel */}
              <Tab.Panel className="rounded-xl bg-white p-2 lg:p-3">
                <div className="overflow-y-auto max-h-[60vh] lg:max-h-[50vh]">
                  {loading ? (
                    <div className="flex justify-center items-center h-32">
                      <div className="animate-spin rounded-full h-6 w-6 lg:h-8 lg:w-8 border-b-2 border-indigo-500"></div>
                    </div>
                  ) : connections.followers.length > 0 ? (
                    <div className="divide-y divide-gray-100">
                      {connections.followers.map((connection) => {
                        const user = connection.follower
                        return (
                          <div key={connection._id} className="flex items-center p-3 lg:p-4 hover:bg-gray-50">
                            <ProfilePhoto src={getProfilePhotoUrl(user)} alt={user.name} size="lg" />
                            <div className="ml-3 lg:ml-4 flex-grow">
                            ...
                            <button
                              onClick={() => navigate(`/profile/${user._id}`)}
                              className="px-2 lg:px-3 py-1 text-xs lg:text-sm bg-indigo-100 text-indigo-700 rounded-md hover:bg-indigo-200"
                            >
                              View Profile
                            </button>
                          </div>
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-600 text-sm lg:text-base">No followers yet</p>
                    </div>
                  )}
                </div>
              </Tab.Panel>
              ...
              <Tab.Panel className="rounded-xl bg-white p-2 lg:p-3">
                <div className="overflow-y-auto max-h-[60vh] lg:max-h-[50vh]">
                  {loading ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-600 text-sm lg:text-base">You are not following anyone yet</p>
                    </div>
                  )}
                </div>
              </Tab.Panel>
              ...
              <Tab.Panel className="rounded-xl bg-white p-2 lg:p-3">
                <div className="overflow-y-auto max-h-[60vh] lg:max-h-[50vh]">
                  {loading ? (
                    ...
                    <div className="divide-y divide-gray-100">
                      {connections.pending.map((request) => (
                        <div key={request._id} className="flex items-center p-3 lg:p-4 hover:bg-gray-50">
                          <ProfilePhoto
                            src={getProfilePhotoUrl(request.follower)}
                            alt={request.follower.name}
                            size="lg"
                          />
                          <div className="ml-3 lg:ml-4 flex-grow">
                            <h3 className="font-medium text-gray-900 text-sm lg:text-base">{request.follower.name}</h3>
                            <p className="text-xs lg:text-sm text-gray-500">{request.follower.email}</p>
                            <p className="text-xs text-gray-500 capitalize">{request.follower.role || ""}</p>
                          </div>
                          <div className="flex flex-col lg:flex-row gap-1 lg:gap-2">
                            <button
                              onClick={() => handleRejectRequest(request._id)}
                              className="px-2 lg:px-3 py-1 text-xs lg:text-sm bg-red-100 text-red-700 rounded-md hover:bg-red-200"
                            >
                              Reject
                            </button>
                            <button
                              onClick={() => handleAcceptRequest(request._id)}
                              className="px-2 lg:px-3 py-1 text-xs lg:text-sm bg-green-100 text-green-700 rounded-md hover:bg-green-200"
                            >
                              Accept
                            </button>
                          </div>
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-600 text-sm lg:text-base">No pending requests</p>
                    </div>
                  )}
                </div>
              </Tab.Panel>
              ...
              <Tab.Panel className="rounded-xl bg-white p-2 lg:p-3">
                <div className="overflow-y-auto max-h-[60vh] lg:max-h-[50vh]">
                  {loading ? (
                    ...
                    <div className="divide-y divide-gray-100">
                      {connections.sent.map((request) => (
                        <div key={request._id} className="flex items-center p-3 lg:p-4 hover:bg-gray-50">
                          ...
                          />
                          <div className="ml-3 lg:ml-4 flex-grow">
                            <h3 className="font-medium text-gray-900 text-sm lg:text-base">{request.following.name}</h3>
                            <p className="text-xs lg:text-sm text-gray-500">{request.following.email}</p>
                            <p className="text-xs text-gray-500 capitalize">{request.following.role || ""}</p>
                          </div>
                          <div className="flex flex-col lg:flex-row gap-1 lg:gap-2">
                            <button
                              onClick={() => handleCancelRequest(request._id)}
                              className="px-2 lg:px-3 py-1 text-xs lg:text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200"
                            >
                              Cancel
                            </button>
                            <button
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-600 text-sm lg:text-base">No sent requests</p>
                    </div>
                  )}
                </div>
              </Tab.Panel>
